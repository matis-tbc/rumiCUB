...
import { api, type GameStateDTO, type SuggestResponse } from "./lib/api";
...
export default function App() {
  ...
  // Bootstrap: check health, resume saved game if any, else create one.
  useEffect(() => {
    (async () => {
      try {
  ...
  async function playSuggested() {
    ...
    setLoading(true);
    try {
      const r = await api.play(state.id, suggestion.new_board);
      setState(r.state);
      setSuggestion(null);
      if (!r.ok) setError(r.reason);
    } catch (e) {
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 flex flex-col xl:flex-row gap-6 p-8 max-w-[1600px] mx-auto w-full">
        ...
        <div className="flex-1 flex flex-col gap-6">
          ...
          <TileRack tiles={current.hand} label="your hand" />
        </div>

        {/* Right: actions + suggestion panel */}
        <aside className="xl:w-80 flex flex-col gap-4">
          ...
          >
            ...
            <div className="flex flex-col gap-2">
              <Btn onClick={draw} disabled={loading || state.is_over}>
                draw a tile
              </Btn>
              <Btn
...
function Btn({
  ...
  return (
    <button
      ...
      disabled={disabled}
      title={title}
      className="w-full px-3 py-2 text-sm uppercase tracking-widest rounded-sm transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
      style={{
...
function CompactTile({ tile }: { tile: import("./lib/api").TileDTO }) {
  // smaller, no shadow — used inside the suggestion box
